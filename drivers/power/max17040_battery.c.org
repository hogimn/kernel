...
static void max17040_get_soc(struct i2c_client *client)
{
	...
	if(chip->soc)	{
		...
		if(chip->vcell >= POWER_OFF_VOLTAGE)	{
			#if defined(MAX17040_DEBUG)
				printk("BATTERY(%s) : Soc(msb = %d, lsb = %d), Vcell = %d\n", __FUNCTION__, msb, lsb, chip->vcell);
			#endif
			chip->soc = 1;
		}
	}
	...
	if((chip->usb_online)||(chip->online)) {
		if(chip->pdata->charger_done()) {
			if((chip->soc >= MAX17040_BATTERY_FULL) && (chip->vcell >= CHARGE_OFF_VOLTAGE))	{
				...
				chip->soc = MAX17040_BATTERY_FULL + 1;
				#if defined(MAX17040_DEBUG)
					printk("CHARGER Done. Soc(%d) or Vcell(%d)!\n", chip->soc, chip->vcell);
				#endif
			}
			else	{
				#if defined(MAX17040_DEBUG)
					printk("CHARGER Done. but Soc(%d) or Vcell(%d) not Full!!. Chaarger Enable!\n", chip->soc, chip->vcell);
				#endif
				
				if (chip->pdata->charger_enable())	{
					chip->status = POWER_SUPPLY_STATUS_CHARGING;
					#if defined(MAX17040_DEBUG)
						printk("CHARGER Enable!\n");
					#endif
				}
				else	{
					#if defined(MAX17040_DEBUG)
						printk("Error! CHARGER Disable!\n");
					#endif
