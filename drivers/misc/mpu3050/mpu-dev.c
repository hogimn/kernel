...
{
int i;
printk("JNJ : MU3050 read : addr=0x%x, reg=0x%x, len=0x%x ",addr, msg.address, msg.length);
for(i=0;i<msg.length;i++) printk("0x%x ", msg.data[i]);
printk(" \r\n");
}
		break;
	case MPU_WRITE:
...
{
int i;
printk("JNJ : MU3050 read mem : addr=0x%x,reg=0x%x ",addr,msg.address);
for(i=0;i<msg.length;i++) printk("0x%x ", msg.data[i]);
printk(" \r\n");
}
		break;
...
{
int i;
printk("JNJ : MU3050 read fifo :addr=0x%x ",addr);
for(i=0;i<msg.length;i++) printk("0x%x ", msg.data[i]);
printk(" \r\n");
}
		break;
...
printk("JNJ : mpu3050_probe -, res=%d \r\n",res);
