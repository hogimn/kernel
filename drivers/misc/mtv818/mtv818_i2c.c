...
{	
	...
	mtv818_cb_ptr->i2c_client_ptr = client;
	mtv818_cb_ptr->i2c_adapter_ptr = to_i2c_adapter(client->dev.parent); 
...
void mtv818_i2c_read_burst(unsigned char reg, unsigned char *buf, int size)
{
	...
	ret = i2c_transfer(mtv818_cb_ptr->i2c_adapter_ptr, msg, 2);
	if (ret != 2) {  
...
#ifndef RTV_DUAL_CHIP_USED
	ret = i2c_transfer(mtv818_cb_ptr->i2c_adapter_ptr, &msg, 1);
	if (ret != 1) {  
