...
static RTV_TDMB_REG_SUBCH_INFO g_atTdmbRegSubchInfo[NUM_ATTECHED_RTV_CHIP][RTV_MAX_NUM_MULTI_SUB_CHANNEL];
...
#if !defined(RTV_TDMB_MULTI_SUB_CHANNEL_ENABLE) /* Single Sub Channel Mode */
...
static void tdmb_OpenSubChannel(UINT nSubChID, E_RTV_TDMB_SERVICE_TYPE eServiceType, UINT nThresholdSize)
{
	...
	else /* Audio/Data */
	{
		...
		/* Set sub channel. */ 
		switch( nHwSubChIdx )
		{	/* MSC1 used for only Video service. */
...
#if defined(RTV_TDMB_MULTI_SUB_CHANNEL_ENABLE) || defined(RTV_IF_MPEG2_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE) || defined(RTV_IF_QUALCOMM_TSIF) || defined(RTV_IF_MPEG2_PARALLEL_TSIF)	
	...
	if(g_aTdmbPrevChFreqKHz[RaonTvChipIdx] == dwChFreqKHz)
	{
		...
		if(eServiceType == RTV_TDMB_SERVICE_VIDEO)
		{
			...
			if((g_aRtvUsedHwSubChIdxBits[RaonTvChipIdx] & TDMB_MSC1_SUBCH_USE_MASK) == TDMB_MSC1_SUBCH_USE_MASK)
			{
				...
				return RTV_NO_MORE_SUB_CHANNEL; // Only 1 Video service. Close other video service.
			}
		}
		else /* Audio/Data */
		{
		...
		RTV_GUARD_LOCK;
		tdmb_OpenSubChannel(nSubChID, eServiceType, RTV_TS_PACKET_SIZE);    
...
#if defined(RTV_TDMB_MULTI_SUB_CHANNEL_ENABLE) /* CIF Mode */
	rtvCIFDEC_Init();	
#endif	
