...
typedef struct
{
	...
	UINT subch_id[RTV_MAX_NUM_MULTI_SUB_CHANNEL];
	UINT subch_size[RTV_MAX_NUM_MULTI_SUB_CHANNEL];

	UINT padding_bytes;
} TSI_CIF_HEADER_INFO;


static U8 temp_ts_buf_pool[2 * (RTV_TDMB_CIF_HEADER_SIZE + 384 + MAX_SUBCH_SIZE_PER_CIF + 188/*padding*/)];
...
{
	UINT mod, i, k, diff;
	U8 header_data[RTV_TDMB_CIF_HEADER_SIZE];

	if((prev_ts_len + cur_ts_len) < RTV_TDMB_CIF_HEADER_SIZE)
	...
	{
		if(prev_ts_len >= RTV_TDMB_CIF_HEADER_SIZE)
		{
		...
		{ 
			diff = RTV_TDMB_CIF_HEADER_SIZE - prev_ts_len;
	...
	hdr_info->pkt_size = RTV_TDMB_CIF_HEADER_SIZE + hdr_info->cif_size; 
...
static PARSER_STATUS_TYPE search_start_cmd(TSI_CIF_HEADER_INFO *hdr_info, const U8 **cur_ts_buf_ptr, UINT *cur_ts_len)
{
	...
	while((g_nPrevTsLen + temp_cur_ts_len) >= RTV_TDMB_CIF_HEADER_SIZE)
	{
	...
	} /* end of while((g_nPrevTsLen + temp_cur_ts_len) >= RTV_TDMB_CIF_HEADER_SIZE) */
...
void rtvCIFDEC_Decode(RTV_CIF_DEC_INFO *ptDecInfo, const U8 *pbTsBuf, UINT nTsLen)
{
	...
	UINT i=0;
	U8 *dest_msc_buf_ptr[RTV_MAX_NUM_MULTI_SUB_CHANNEL];
	...
	do
	{
		...
		/* Remove the header. */
		pull_packet(&cur_ts_buf_ptr, &nTsLen, RTV_TDMB_CIF_HEADER_SIZE);
		...
		for(i=0; i<RTV_MAX_NUM_MULTI_SUB_CHANNEL; i++) // SUBCHx ID
		{		
			if(hdr_info.subch_size[i] != 0) 
			{
...
#else
/* SPI */
static UINT g_nUsedSubchOutBufIdxBits; /* 0 ~ (RTV_MAX_NUM_MULTI_SUB_CHANNEL-1) */
...
//	RTV_DBGMSG1("\n[rtvCifParser_Parsing] msc_total: %d\n", nTsLen);
	...
	do
	{
		...
		if(ch_length < RTV_TDMB_CIF_HEADER_SIZE) 
		{
			RTV_DBGMSG0("[CIF Parser] Invalid ch length. May be broken...\n"); 
			return;
		}

		subch_size = ch_length - RTV_TDMB_CIF_HEADER_SIZE;
		data_ptr   = cif_header_ptr + RTV_TDMB_CIF_HEADER_SIZE;   

		/* Verify a Header */
		if(subch_size > MAX_SUBCH_SIZE_PER_CIF) 
		{
			RTV_DBGMSG0("[CIF Parser] Invalid sub ch SIZE.\n"); 
			return;
		}
		...
		if(subch_size != 0) // 
		{
			...
			{
				RTV_DBGMSG0("[CIF Parser] Invalid sub ch ID.\n"); 
				return;
			}
		}
