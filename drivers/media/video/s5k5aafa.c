	/* linux/drivers/media/video/s5k5aafa.c
 *
...
/* Default resolution & pixelformat. plz ref s5k5aafa_platform.h */
...
/*
 * S5K5AAFA register structure : 2bytes address, 2bytes value
 * retry on write failure up-to 5 times
 */
static inline int s5k5aafa_write(struct v4l2_subdev *sd, u8 addr, u8 val)
{
...
static int s5k5aafa_i2c_write(struct v4l2_subdev *sd, unsigned char i2c_data[],
...
static const char *s5k5aafa_querymenu_wb_preset[] = {
	"WB Tungsten", "WB Fluorescent", "WB sunny", "WB cloudy", NULL
};

static const char *s5k5aafa_querymenu_effect_mode[] = {
	"Effect Sepia", "Effect Aqua", "Effect Monochrome",
	"Effect Negative", "Effect Sketch", NULL
};

static const char *s5k5aafa_querymenu_ev_bias_mode[] = {
...
static struct v4l2_queryctrl s5k5aafa_controls[] = {
	...
	},
	{
		...
		.minimum = 0,
		.maximum = ARRAY_SIZE(s5k5aafa_querymenu_wb_preset) - 2,
		.step = 1,
		.default_value = 0,
	},
	{
	...
	},
	{
		...
		.name = "Exposure bias",
		.minimum = 0,
		.maximum = ARRAY_SIZE(s5k5aafa_querymenu_ev_bias_mode) - 2,
		.step = 1,
		.default_value = \
			(ARRAY_SIZE(s5k5aafa_querymenu_ev_bias_mode) - 2) / 2,
			/* 0 EV */
	},
	{
		.id = V4L2_CID_COLORFX,
		...
		.name = "Image Effect",
		.minimum = 0,
		.maximum = ARRAY_SIZE(s5k5aafa_querymenu_effect_mode) - 2,
		.step = 1,
		.default_value = 0,
	},
	{
...
static int s5k5aafa_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
		...
		(unsigned char *) s5k5aafa_regs_ev_bias[ctrl->value], \
			sizeof(s5k5aafa_regs_ev_bias[ctrl->value]));
		break;
...
static int s5k5aafa_init(struct v4l2_subdev *sd, u32 val)
{
	...
	for (i = 0; i < S5K5AAFA_INIT_REGS; i++) {

		_tmp[0] =(unsigned char)( s5k5aafa_init_reg_short[i] >> 8) ;
		_tmp[1] =(unsigned char)( s5k5aafa_init_reg_short[i] & 0xff);

		err = s5k5aafa_i2c_write(sd,_tmp , 2);
			 ...
			 const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "s5k5aafa has been probed\n");
	return 0;
}
...
MODULE_DESCRIPTION("Samsung Electronics S5K5AAFA XGA camera driver");
MODULE_AUTHOR("Dongyol.Yun <ydongyol@hardkernel.com>");
MODULE_LICENSE("GPL");
