...
struct s5k4ea_enum_framesize s5k4ea_framesize_list[] = {
	 { S5K4EA_PREVIEW_VGA,  640,  480 } 
	 //{ S5K4EA_PREVIEW_XVGA,  1024,  768 } 
	// { S5K4EA_PREVIEW_WVGA,  800,  480 }, 
	//JNJ { S5K4EA_PREVIEW_HD1080, 1920, 1080 }
};*/
// ~KSJ
...
static int s5k4ea_set_frame_size(struct v4l2_subdev *sd,int width)
{
	...
	if(width == 2048)
	{
		//err = __s5k4ea_init_2bytes(sd, \
    	  //(unsigned short **) s5k4ea_jpeg_capture_5m, S5K4EA_JPEG_CAPTURE_5M);
		err = __s5k4ea_init_2bytes(sd, \
    	  (unsigned short **) s5k4ea_capture_3m, S5K4EA_CAPTURE_3M);
		//s5k4ea_init_reg_total[1855][1]=2048;
		//s5k4ea_init_reg_total[1856][1]=1536;
		//err=s5k4ea_write_array(sd,s5k4ea_init_reg_total,S5K4EA_INIT_REGS_TOTAL);
		//err=s5k4ea_write_array(sd,s5k4ea_capture_preset,S5K4EA_CAPTURE_PRESET);
		mdelay(100);
	}
	else if(width == 640)
	{
		err=s5k4ea_write_array(sd,s5k4ea_preview_640x480,S5K4EA_PREVIEW_640x480);
		//s5k4ea_init_reg_total[1812][1]=1024;
		//s5k4ea_init_reg_total[1813][1]=768;
		//err=s5k4ea_write_array(sd,s5k4ea_init_reg_total,S5K4EA_INIT_REGS_TOTAL);
	}
...
//#ifdef S5K4EA_COMPLETE
	...
	case V4L2_CID_CAMERA_CAPTURE : //KSJ
		...
		err=s5k4ea_write_array(sd, s5k4ea_jpeg_capture_5m, S5K4EA_JPEG_CAPTURE_5M);
		//err=s5k4ea_write_array(sd,s5k4ea_preview_3m,S5K4EA_CAPTURE_3M);
		break;
...
static int s5k4ea_wakeup(struct v4l2_subdev *sd)
{
	...
	err = __s5k4ea_init_2bytes(sd, \
    	  (unsigned short **) s5k4ea_preview_640x480, S5K4EA_PREVIEW_640x480);
