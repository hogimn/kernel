...
void s5p_vp_ctrl_stop(void)
{
...
#endif
	{
        ...
        s5pv310_cpufreq_lock_free(DVFS_LOCK_ID_TV);
        s5pv310_busfreq_lock_free(DVFS_LOCK_ID_TV);
#endif
...
int s5p_vp_ctrl_start(void)
{
	...
	if (s5p_vp_ctrl_private.running)
...
#endif
		{
	                ...
	                {
				//if(s5pv310_cpufreq_lock(DVFS_LOCK_ID_TV, CPU_L0) || s5pv310_busfreq_lock(DVFS_LOCK_ID_TV, BUS_L0))
				if(s5pv310_busfreq_lock(DVFS_LOCK_ID_TV, BUS_L0))
