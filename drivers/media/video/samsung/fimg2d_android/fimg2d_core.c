...
int g2d_dynamic_freq_lock(struct g2d_global *g2d_dev)
{
	struct timeval previous_marker;

	previous_marker = g2d_dev->freq_lock.cur_marker;
	do_gettimeofday(&g2d_dev->freq_lock.cur_marker);

	if (GET_USEC(previous_marker, g2d_dev->freq_lock.cur_marker) > 100000) {
		g2d_dev->freq_lock.cnt = 0;
		return false;
	} else {
		if (g2d_dev->freq_lock.cnt == 0) {
			g2d_dev->freq_lock.start_marker = g2d_dev->freq_lock.cur_marker;
		}
		g2d_dev->freq_lock.cnt++;

		if (g2d_dev->freq_lock.cnt > 5) {
			if (GET_USEC(g2d_dev->freq_lock.start_marker, g2d_dev->freq_lock.cur_marker) > 16700*2)
				return true;
		}
	}
	return false;
