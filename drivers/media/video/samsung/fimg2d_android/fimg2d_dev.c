...
static int g2d_release(struct inode *inode, struct file *file)
{
	if (atomic_read(&g2d_dev->in_use) == 0) {
		...
		if (atomic_read(&g2d_dev->freq_cpulock_flag) == 1) {
			s5pv310_cpufreq_lock_free(DVFS_LOCK_ID_G2D);
			atomic_set(&g2d_dev->freq_cpulock_flag, 0);
		}

		if (atomic_read(&g2d_dev->freq_buslock_flag) == 1) {
			s5pv310_busfreq_lock_free(DVFS_LOCK_ID_G2D);
			atomic_set(&g2d_dev->freq_buslock_flag, 0);
		}
#endif		
	}
...
#if defined(CONFIG_CPU_FREQ)
		if (g2d_dynamic_freq_lock(g2d_dev)) {
			if (atomic_read(&g2d_dev->freq_cpulock_flag) == 0) {
				atomic_set(&g2d_dev->freq_cpulock_flag, 1);
				if (s5pv310_cpufreq_lock(DVFS_LOCK_ID_G2D, CPU_L1))
					FIMG2D_ERROR("error : failed CPU lock DVFS\n");
			}

			if (atomic_read(&g2d_dev->freq_buslock_flag) == 0) {
				atomic_set(&g2d_dev->freq_buslock_flag, 1);
				if (s5pv310_busfreq_lock(DVFS_LOCK_ID_G2D, BUS_L0))
					FIMG2D_ERROR("error : failed BUS lock DVFS\n");
			}
		}
#endif
