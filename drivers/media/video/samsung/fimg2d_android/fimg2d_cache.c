...
/* L2 cache all flush threshold time is 200ms */
#define L2_ALL_THRESHOLD_TIME 200000
#define L2_ALL_THRESHOLD_COUNT 3
...
void g2d_mem_outer_cache(struct g2d_global *g2d_dev, g2d_params * params, int *need_dst_clean)
{
	...
	if ((src_size +dst_size) >  L2_ALL_THRESHOLD_SIZE) {
		previous_time = g2d_dev->flush_mark.cur_marker;
		do_gettimeofday(&g2d_dev->flush_mark.cur_marker);

		if (GET_USEC(previous_time, g2d_dev->flush_mark.cur_marker) < L2_ALL_THRESHOLD_TIME)
			g2d_dev->flush_mark.cnt++;
		else
			g2d_dev->flush_mark.cnt = 0;

		if (g2d_dev->flush_mark.cnt > (L2_ALL_THRESHOLD_COUNT - 1)) {
			outer_flush_all();
			*need_dst_clean = true;
			return;
		}
